...
var net = require("net"), util = require("util"), Queue = require("./lib/queue").Queue, events = require("events"), parsers = [], default_port = 1984, default_host = "127.0.0.1", default_options = {
	"username" : "admin",
	"password" : "admin"
};
...
function BasexClient(stream, options) {
	...
	if (self.options.parser) {
		if (!parsers.some(function(parser) {
			...
			throw new Error("Couldn't find named parser " + self.options.parser
					+ " on this system");
		}
	} else {
		if (exports.debug_mode) {
...
BasexClient.prototype.on_connect = function() {
	...
	if (exports.debug_mode) {
		console.log("Sending auth to " + this.host + ":" + this.port + " fd "
				+ this.stream.fd);
	}
...
BasexClient.prototype.ready_check = function() {
	...
	function send_info_cmd() {
		...
		self.send_anyway = true; // secret flag to send_command to send
									// something even if not "ready"
		self.info(function(err, res) {
			...
			if (!obj["loading"] || (obj["loading"] && obj["loading"] == 0)) {
				...
				if (exports.debug_mode) {
					console.log("basex server still loading, trying again in "
							+ retry_time);
				}
...
BasexClient.prototype.return_error = function(err) {
	...
	if (command_obj && typeof command_obj.callback === "function") {
		try {
			...
			// if a callback throws an exception, re-throw it on a new stack so
		...
		// this will probably not make it anywhere useful, but we might as well
		// throw
		process.nextTick(function() {
			throw err;
		});
...
BasexClient.prototype.return_reply = function(reply) {
	var command_obj = this.command_queue.shift(), obj, i, len, key, val, type, timestamp, args;
    console.log("got reply",reply,command_obj);
	if (this.subscriptions === false && this.command_queue.length === 0) {
		this.emit("idle");
		this.command_queue = new Queue();
	}
	...
	if (command_obj && !command_obj.sub_command) {
		if (typeof command_obj.callback === "function") {
			...
			try {
				...
				// if a callback throws an exception, re-throw it on a new stack
				// so the parser can keep going
				process.nextTick(function() {
					throw err;
				});
			}
		} else if (exports.debug_mode) {
			console.log("no callback for reply: "
					+ (reply && reply.toString && reply.toString()));
		}
		...
		if (Array.isArray(reply)) {
			...
			if (type === "message") {
				this.emit("message", reply[1].toString(), reply[2]); // channel,
																		// message
			} else if (type === "pmessage") {
				this.emit("pmessage", reply[1].toString(), reply[2].toString(),
						reply[3]); // pattern, channel, message
			} else if (type === "subscribe" || type === "unsubscribe"
					|| type === "psubscribe" || type === "punsubscribe") {
				if (reply[2] === 0) {
					this.subscriptions = false;
					if (this.debug_mode) {
						console
								.log("All subscriptions removed, exiting pub/sub mode");
					}
				}
				this.emit(type, reply[1].toString(), reply[2]); // channel,
																// count
			} else {
				throw new Error(
						"subscriptions are active but got unknown reply type "
								+ type);
			}
		} else {
			throw new Error(
					"subscriptions are active but got an invalid reply: "
							+ reply);
		}
		...
		throw new Error(
				"node_basex command queue state error. If you can reproduce this, please report it.");
	}
};
